...
var original_top_search_val,
    ...
    _country = document.getElementById("company_default_country")?.value;
...
$(function () {
    ...
    $("body").on("click", ".transaction-submit", function () {
        ...
        if (form.valid()) {
            ...
            } else if (that.hasClass("create-eway-bill")) {
                form.append(hidden_input("create_eway_bill", "true"));
            } else if (that.hasClass("create-einvoice")) {
                form.append(hidden_input("create_einvoice", "true"));
            }
        }
        form.submit();
    });
    ...
    $("body").on("change", ".f_client_id #clientid", function () {
        ...
        requestGetJSON(
            ...
            console.log('response taxes', response.taxes)
            if (response.taxes){
                $('#main_tax').html(response.taxes).selectpicker('refresh')
            }
...
function add_item_to_preview(id) {
    requestGetJSON("invoice_items/get_item_by_id/" + id).done(function (
        ...
        $('.main input[name="hsnCode"]').val(response.hsnCode);
...
function add_item_to_table(data, itemid, merge_invoice, bill_expense) {
    ...
    get_taxes_dropdown_template(tax_name, data.taxname).done(function (
        ...
        // order input
        console.log('tax_dropdown ++', tax_dropdown)
        table_row +=
        ...
        console.log("country code", document.getElementById("company_default_country")?.value);
        if (_country == '102'){
            document.getElementById("company_default_country")?.value
            table_row += '<td class="" align="right"><input type="text" name="newitems[' +
                item_key +
                '][hsnCode]" value="' +
                data.hsnCode +
            '" class="form-control"></td>';
            table_row += '<td class="tax_igst" align="right"></td>';
            table_row += '<td class="tax_sgst" align="right"></td>';
            table_row += '<td class="tax_cgst" align="right"></td>';
            table_row += '<td class="taxable_amount" align="right">Taxable Amount</td>';
        }
        ...
        setTimeout(function () {
            calculate_total();
            console.log('its here to empty main tax')
            $('.main-tax').selectpicker("val", '');
        }, 15);
        ...
        if (_country == '102') {
            $('.supply_type').trigger('change')
            // document.getElementById("supply_type").value
        }
        return true;
    });
...
function get_item_preview_values() {
    ...
    response.hsnCode = $('.main input[name="hsnCode"]').val();
    return response;
}
...
function calculate_total() {
    ...
    $.each(rows, function () {
        ...
        console.log('item_taxes', item_taxes)
        if (item_taxes) {
            $.each(item_taxes, function (i, taxname) {
                console.log('item_taxes i', i)
                console.log('taxname ++', taxname)
                tax_name_price = taxname.split("-")[0];
                console.log('tax_name_price ====', tax_name_price)
                taxrate = row.find('select.tax [value="' + taxname + '"]').data("taxrate");
                calculated_tax = (_amount / 100) * taxrate;
                taxable_amount += calculated_tax;
                if (_country == '102') {
                    row.find('td.tax_' + tax_name_price.toLowerCase()).text(calculated_tax)
                }
                if (!taxes.hasOwnProperty(taxname)) {
                    if (taxrate != 0) {
                        _tax_name = taxname.split("|");
                        console.log("_tax_name",_tax_name);
                        tax_row =
        ...
        if (_country == '102') {
            console.log('taxes[taxname] here in if', taxable_amount)
            $(this).find("td.taxable_amount").html(format_money(taxable_amount, true));
        }
    });
