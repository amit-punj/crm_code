...
$config['useragent'] = get_option('saas_mail_engine'); // phpmailer or codeigniter

$microsoftClientId = get_option('saas_microsoft_mail_client_id');
$microsoftClientSecret = get_instance()->encryption->decrypt(get_option('saas_microsoft_mail_client_secret'));
$tenantId = get_option('saas_microsoft_mail_azure_tenant_id');

$googleClientId = get_option('saas_google_mail_client_id');
$googleClientSecret = get_instance()->encryption->decrypt(get_option('saas_google_mail_client_secret'));

if (!empty($microsoftClientId) && !empty($microsoftClientSecret) && get_option('saas_email_protocol') == 'microsoft') {
    $config['client_id'] = $microsoftClientId;
    $config['client_secret'] = $microsoftClientSecret;
    $config['tenant_id'] = $tenantId;
    $config['refresh_token'] = get_option('saas_microsoft_mail_refresh_token');
}

if (!empty($googleClientId) && !empty($googleClientSecret) && get_option('saas_email_protocol') == 'google') {
    $config['client_id'] = $googleClientId;
    $config['client_secret'] = $googleClientSecret;
    $config['refresh_token'] = get_option('saas_google_mail_refresh_token');
}

$config['protocol'] = get_option('saas_email_protocol');
$config['mailpath'] = '/usr/bin/sendmail'; // or "/usr/sbin/sendmail"
$config['smtp_host'] = trim(get_option('saas_smtp_host'));

if (get_option('saas_smtp_username') == '') {
    $config['smtp_user'] = trim(get_option('saas_smtp_email'));
} else {
    $config['smtp_user'] = trim(get_option('saas_smtp_username'));
}

$config['smtp_pass'] = get_instance()->encryption->decrypt(get_option('saas_smtp_password'));
$config['smtp_port'] = trim(get_option('saas_smtp_port'));
$config['smtp_timeout'] = 30;
$config['smtp_crypto'] = get_option('saas_smtp_encryption');
...
$charset = strtoupper(get_option('saas_smtp_email_charset'));
