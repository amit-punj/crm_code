                            ...
                            foreach ($all_packages as $v_package) {
                                ...
                                if (isset($package_id)) {
                                    if ($package_id == $v_package->id) {
                                     ...
                                     data-subtext="<?php echo _l('monthly') . ': ' . $v_package->monthly_price . ' ' . _l('yearly') . ': ' . $v_package->yearly_price . ' ' . _l('lifetime') . ': ' . $v_package->lifetime_price . ' ' . strip_tags(mb_substr(!empty($c_pricing->description) ? $c_pricing->description : '', 0, 200)) . '...'; ?>"><?php echo $v_package->name; ?></option>
                            <?php } ?>
                            <?php
                        }
                        ?>
                ...
                <div class="form-group mb-3">
                    <select name="country" class="form-control select_box"
                        ...
                        if (!empty($countries)): foreach ($countries as $key => $country): ?>
                            <option
                                    value="<?= $country['short_name'] ?>" <?= (!empty($company_info->country) && $company_info->country == $country['short_name'] || get_option('company_country') == $country['short_name'] ? 'selected' : NULL) ?>><?= $country['short_name'] ?>
                            </option>
                        <?php
                        endforeach;
                        endif;
