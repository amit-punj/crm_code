...
class Companies extends AdminController
{
    ...
    public function companiesList($status = null)
    {
        if ($this->input->is_ajax_request()) {
            ...
            $this->datatables->select = 'tbl_saas_companies_history.package_name,tbl_saas_companies_history.id as company_history_id,tbl_saas_companies.*';
            $this->datatables->join_table = array('tbl_saas_companies_history');
            $this->datatables->join_where = array('tbl_saas_companies_history.companies_id=tbl_saas_companies.id AND tbl_saas_companies_history.active=1');
...
//            $this->datatables->groupBy = array('tbl_saas_companies.id');
            ...
            foreach ($fetch_data as $key => $row) {
                ...
                $sub_array[] = '<a href="' . base_url() . 'saas/gb/package_details/' . $row->company_history_id . '/1" title="' . _l('details') . '" data-toggle="modal" data-target="#myModal">' . $row->package_name . '</a>  ';
                ...
                $sub_array[] = '<span class="label label-' . $label . '">' . _l($row->status) . '</span>';
                $sub_array[] = _dt($row->created_date);
                // customize package
                $action .= '<a href="' . saas_url('packages/customize/' . $row->id) . '" class="btn btn-primary "> <i class="fa fa-cog"></i> ' . _l('customize') . '</a>';
                $sub_array[] = $action;
                $data[] = $sub_array;
            }
            render_table($data);
    ...
    public function reset_password($id)
    {
        if ($this->session->userdata('user_type') == 4) {
            ...
            if (!empty($new_password)) {
                ...
                $user_info = $this->db->where('user_id', get_staff_user_id())->get('tbl_users')->row();
                $old_password = $this->saas_model->hash($old_password);
                if ($user_info->password == $old_password) {
                    $where = array('id' => $id);
                    $action = array('password' => $this->saas_model->hash($new_password));
                    // update password into tbl_saas_companies
                    ...
                    if (!empty($company_info->db_name)) {
                        ...
                        if (!empty($this->db)) {
                            ...
                            $this->db->update('tbl_users', array('password' => $this->saas_model->hash($new_password)));
                        }
                    }
                    $type = "success";
                    $message = _l('message_new_password_sent');
                } else {
                    $type = "error";
