        ...
        $menu['packages'] = array(
            ...
            'children' => array(
                ...
                array(
                    ...
                    'href' => saas_url('packages/settings/modules'),
                    'position' => 3,
                    'icon' => '',
                ),
                // settings
...
//                ),
    ...
    public function company_info($id, $full = null)
    {
        ...
        return $this->select_data('tbl_saas_companies', $select, NULL, array('tbl_saas_companies.id' => $id, 'tbl_saas_companies_history.active' => 1), ['tbl_saas_companies_history' => 'tbl_saas_companies.id = tbl_saas_companies_history.companies_id'], 'row');
    }
    ...
    private function install_basic_data($companyInfo, $fresh_db)
    {
        ...
        $package_info = get_row('tbl_saas_packages', array('id' => $companyInfo->package_id));
        $modules = $package_info->modules;
        $all_modules = array();
        $mTable = db_prefix() . 'modules';
        if (!empty($modules)) {
            $modulesInfo = unserialize($modules);
            // $modulesInfo push new modules name is saas
            $modulesInfo[] = 'saas';
        }
        if (!empty($modulesInfo)) {
            foreach ($modulesInfo as $module) {
                $module = get_row($mTable, array('module_name' => $module));
                if (!empty($module)) {
                    $this->db->insert($db_name . '.' . $mTable, array(
        ...
        if (!empty($_POST['smtp_host'])) {
            $this->db->query("UPDATE `" . $db_name . "`.`" . db_prefix() . "options` SET `value` = '" . $_POST['protocol'] . "' WHERE `" . db_prefix() . "options`.`name` = 'email_protocol'");
            $this->db->query("UPDATE `" . $db_name . "`.`" . db_prefix() . "options` SET `value` = '" . $_POST['smtp_host'] . "' WHERE `" . db_prefix() . "options`.`name` = 'smtp_host'");
            $this->db->query("UPDATE `" . $db_name . "`.`" . db_prefix() . "options` SET `value` = '" . $_POST['smtp_port'] . "' WHERE `" . db_prefix() . "options`.`name` = 'smtp_port'");
            $this->db->query("UPDATE `" . $db_name . "`.`" . db_prefix() . "options` SET `value` = '" . $_POST['smtp_user'] . "' WHERE `" . db_prefix() . "options`.`name` = 'smtp_username'");
            $this->db->query("UPDATE `" . $db_name . "`.`" . db_prefix() . "options` SET `value` = '" . $companyInfo->email . "' WHERE `" . db_prefix() . "options`.`name` = 'smtp_email'");
            $this->db->query("UPDATE `" . $db_name . "`.`" . db_prefix() . "options` SET `value` = '" . $_POST['smtp_pass'] . "' WHERE `" . db_prefix() . "options`.`name` = 'smtp_password'");
            $this->db->query("UPDATE `" . $db_name . "`.`" . db_prefix() . "options` SET `value` = '" . $_POST['smtp_encryption'] . "' WHERE `" . db_prefix() . "options`.`name` = 'smtp_encryption'");
        }
        return true;
    }
    ...
    function create_tables($companyInfo, $fresh_db = null)
    {
        ...
        $tables = array_diff($tables, array(
            'tbl_saas_all_heading_section',
            'tbl_saas_all_section_area',
            'tbl_saas_companies',
            ...
            'tbl_saas_coupon',
            'tbl_saas_affiliates',
            'tbl_saas_affiliate_payouts',
            'tbl_saas_affiliate_users',
            'tbl_saas_front_cms_media',
            'tbl_saas_front_contact_us',
            'tbl_saas_front_menus',
            'tbl_saas_front_menu_items',
            'tbl_saas_front_pages',
            'tbl_saas_front_pages_contents',
            'tbl_saas_front_slider',
            'tbl_saas_package_field',
            'tbl_saas_packages',
            'tbl_saas_menu',
            'tbl_saas_domain_requests',
            'tbl_saas_package_module',
            'tbl_saas_temp_payment',
        ));
        $db_name = $companyInfo->db_name;
        $old_db_name = $this->db->database;
...
//            $this->db->query("DROP TABLE IF EXISTS `" . $db_name . "`.`" . $table . "`");
    ...
    function createPleskDatabase($company_info, $test = null)
    {
        ...
        if (!empty($test)) {
            ...
            $databasename = $company_info->domain . '_' . $company_info->id . '_db';
    ...
    function send_activation_token_email($id)
    {
        ...
        try {
            ...
            send_mail_template('Saas_token_activate_account', SaaS_MODULE, $company_info->email, $company_info->id, $company_info);
            return true;
        } catch (Exception $e) {
        ...
        if (!empty($companyInfo->db_name)) {
            ...
            if (!empty($new_modules) && count($new_modules) > 0) {
                ...
                foreach ($new_modules as $new_module) {
                    $installed_version = $this->app_modules->get($new_module)['headers']['version'];
    ...
    function send_email_to_company($company_id)
    {
        ...
        try {
            send_mail_template('Saas_assign_new_package', SaaS_MODULE, $company_info->email, $company_info->id, $company_info);
            return true;
        } catch (Exception $e) {
            echo 'Message: ' . $e->getMessage();
        }
    ...
    public function getAffiliateUser($id)
    {
        ...
        $this->db->from('tbl_saas_affiliate_users');


        $this->db->join('tbl_saas_affiliate_payouts', 'tbl_saas_affiliate_payouts.user_id=tbl_saas_affiliate_users.user_id', 'left');
        $this->db->join('tbl_saas_companies', 'tbl_saas_affiliate_users.user_id=tbl_saas_companies.referral_by', 'left');
        $this->db->where('tbl_saas_affiliate_users.user_id', $id);
        $this->db->group_by('tbl_saas_affiliate_users.user_id');
        $query = $this->db->get();
        $result = $query->row();
        $total = sum_from_table('tbl_saas_affiliates', ['field' => 'get_amount', 'where' => ['referral_by' => $id]]);
        if ($total == null) {
            $total = 0;
        }
