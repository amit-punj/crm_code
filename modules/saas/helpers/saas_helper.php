...
function is_affiliate($staffid = '')
{
    ...
    $result = $CI->db->count_all_results('tbl_saas_affiliate_users') > 0 ? true : false;
...
function saas_packege_list($info, $limit = null, $front = null): string
{
        ...
        foreach ($all_field as $v_fileds) {
            ...
            if ($v_fileds->field_type == 'text') {
                ...
                if (!empty($value) && $value == 'Yes') {
                    $html .= '<i class="fa fa-check" style="color: #3378ff"></i>' . _l($v_fileds->field_label);
                } else {
                    $html .= '<i class="fa fa-times" style="color: red"></i><del>' . _l($v_fileds->field_label) . '</del>';
                }
            } else if ($v_fileds->field_type == 'numeric') {
            }
            $html .= '</li>';
    ...
    if (!empty($all_module)) {
        $html .= '<li class="' . $liClass . '"
           data-toggle="tooltip" data-placement="top" title="' . implode(', ', $all_module) . '"
>';
...
function saas_set_upload_path_by_type($path)
{
    $subdomain = subdomain();
    if (!empty($subdomain)) {
        $path = str_replace('uploads/', 'uploads/' . $subdomain . '/', $path);
        $dir = $subdomain;
        // create folder if not exists into uploads
        $new_path = FCPATH . 'uploads/' . $dir;
        ...
        if ($result['used_space'] <= $result['total_space']) {
...
function saas_payment_recorded($payment)
{
    ...
    if (!empty($sessionData['companies_id'])) {
        $companies_id = $sessionData['companies_id'];
        $company = get_old_result('tbl_saas_companies', array('id' => $companies_id), false);
    } else {
    ...
    if (!empty($companies_id)) {
        $checkTemp = get_old_result('tbl_saas_temp_payment', array('companies_id' => $companies_id), false);
        if (!empty($checkTemp) && $checkTemp->invoice_id == $payment['invoiceid']) {
            if (round($payment['amount']) !== round($checkTemp->amount)) {
                ...
                if (!empty($r)) {
                    set_alert('success', _l('package_updated_successfully'));

                    $invoice = $CI->db->get_where(db_prefix() . 'invoices', array('id' => $payment['invoiceid']))->row();

                    // delete client
                    $CI->db->where('userid', $invoice->clientid);
                    $CI->db->delete(db_prefix() . 'clients');

                    // delete invoice
                    $CI->load->model('invoices_model');
                    $CI->invoices_model->delete($payment['invoiceid']);

                    // delete temp payment
                    $CI->_table_name = 'tbl_saas_temp_payment';
                    $CI->saas_model->delete_old(array('companies_id' => $companies_id));

                    redirect(companyUrl($company->domain) . 'admin/dashboard');
                }
            }
        }
    }
