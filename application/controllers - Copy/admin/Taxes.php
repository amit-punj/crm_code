...
class Taxes extends AdminController
{
    ...
    public function index()
    {
        ...
        if(get_option('company_default_country') == 102){
            $data['indian_taxes'] = $this->taxes_model->get_indian_taxes();
        } else {
    ...
    /* Enable and Disable Taxes */
    public function enableDisable(){
        if ($this->input->post()) {
            $group_id = $this->input->post('grp_id');
            $data['enable'] = $this->input->post('enable');
            // pr($data);
            $this->db->update( db_prefix() . 'taxes', ['enable'=>0] ); 
            $this->db->where(['grp_id'=> $group_id]);
            $result = $this->db->update( db_prefix() . 'taxes', $data );  
