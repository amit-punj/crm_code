                            ...
                            <div class="row" id="deals-table">
                                <div class="col-md-12">
                                    <div class="row">
                                        ...
                                        <form action="<?= admin_url('gst/gstr1cdnrinvoices') ?>" method="post">
...
<script>
    ...
    function GetReport(){
        ...
        if(year == ''){
            ...
            $.ajax({
                url: '<?php echo admin_url('gst/get_gstr1cdnrinvoices'); ?>',
                data: { 
                    'year': year,
                    'month': month,
                },
                ...
                success: function (data){
                    ...
                    if(data.status){
                        ...
                        data.data.forEach(item => {
                            ...
                            html += '<tr>';
                            html +=`<td>${item.id}</td>`
                            html +=`<td>${item.ctin}</td>`
                            html +=`<td>${item.cfs}</td>`
                            html +=`<td>${item.chksum}</td>`
                            html +=`<td>${item.updby}</td>`
                            html +=`<td>${item.inum}</td>`
                            html +=`<td>${item.idt}</td>`
                            html +=`<td>${item.val}</td>`
                            html +=`<td>${item.pos}</td>`
                            html +=`<td>${item.rchrg}</td>`
                            html +=`<td>${item.etin}</td>`
                            html +=`<td>${item.inv_typ}</td>`
                            html +=`<td>${item.cflag}</td>`
                            html +=`<td>${item.diff_percent}</td>`
                            html +=`<td>${item.opd}</td>`
                            html +=`<td>${item.srctyp}</td>`
                            html +=`<td>${item.irn}</td>`
                            html +=`<td>${item.irngendate}</td>`
                            html +=`<td>${item.num}</td>`
                            html +=`<td>${item.rt}</td>`
    ...
    function DownloadAPIData(){
        ...
        if(year == ''){
            ...
            $.ajax({
                url: '<?php echo admin_url('gst/GetCDNRInvoices'); ?>',
                data: { 
                    'year': year,
                    'month': month,
                },
