...
class Settings_model extends App_Model
{
    ...
    function gst_update($data, $id='')
    {
        // pr('inside gst_update model function');
        // pr($data);
        // pr('btween');
        // pr($id);
        // pr('before if');
        if($id == '') {
            ...
            $this->db->insert( db_prefix() . '_gsts' , $data);
            ...
            $this->db->where('id', $id);
            $result = $this->db->update( db_prefix() . '_gsts', $data );  
            // pr($this->db->last_query(),1); 
